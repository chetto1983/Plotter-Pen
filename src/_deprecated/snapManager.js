...
import { createMagnetHelper } from "./magnetSnap.js";

export function createSnapManager(state) {
  const magnetHelper = createMagnetHelper(state);

  function markAnchorsDirty() {
    state.snap.anchorsDirty = true;
    magnetHelper.invalidate();
  }
  ...
  function applySnap(rawPos) {
    ...
    const magnetSnap = magnetHelper.findMagnet(rawPos);
    ...
    if (anchorSnap) {
      candidates.push({
        priority: 0,
        point: { x: anchorSnap.x, y: anchorSnap.y },
        info: {
    ...
    if (magnetSnap) {
      const basePriority = magnetSnap.type === 'intersection' ? 1 : 2;
      candidates.push({
        priority: basePriority,
        point: { x: magnetSnap.point.x, y: magnetSnap.point.y },
        info: {
          type: magnetSnap.type,
          active: true,
          x: magnetSnap.point.x,
          y: magnetSnap.point.y,
          rawX: rawPos.x,
          rawY: rawPos.y
        }
      });
    ...
    if (gridResult.info.type === 'grid') {
      candidates.push({
        priority: 3,
        point: gridResult.snapped,
        info: gridResult.info
      });
    } else if (gridResult.info.type === 'none') {
      candidates.push({
        priority: 4,
        point: { x: rawPos.x, y: rawPos.y },
        info: gridResult.info
      });
    }
    ...
    if (candidates.length > 0) {
      candidates.sort((a, b) => {
        ...
        const da = distance(a.point, rawPos);
        const db = distance(b.point, rawPos);
        return da - db;
      });
  ...
  function updateSnapUI(evt, rawPos, snapResult) {
    ...
    const collision = state.snap.lastCollision;
    if (state.elements.snapDisplay) {
      if (info && info.active) {
        const label = resolveSnapLabel(info.type);
        let text = `Snap: ${label} (${info.x.toFixed(2)}, ${info.y.toFixed(2)})`;
        if (collision && collision.active) {
          text += ' | Collisione';
        }
        state.elements.snapDisplay.textContent = text;
      } else {
        const base = 'Snap: Nessuno';
        state.elements.snapDisplay.textContent = collision && collision.active ? `${base} | Collisione` : base;
      }
    }
  ...
  return {
    ...
    hideSnapUI,
    resolveSnapLabel,
    detectCollision: magnetHelper.detectCollision
  };
}
