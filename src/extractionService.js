...
function determineArcDirection(center, start, end, referencePoints = [], midPoint = null) {
  ...
  if (resolved && resolved.point) {
    ...
    if (Number.isFinite(resolved.signedDistance)) {
      if (resolved.signedDistance > 0) {
        return 'CCW';
      }
      if (resolved.signedDistance < 0) {
        return 'CW';
  ...
  if (center && start && end) {
    ...
    const midpointToCenter = {
      x: center.x - chordMidpoint.x,
      y: center.y - chordMidpoint.y
    };
    ...
    if (chordLengthSquared < 1e-12) {
      return fallbackToReferencePoints(center, start, end, referencePoints);
    }

    const leftPerpendicularVector = {
      x: -chordVector.y,
      y: chordVector.x
    };

    const perpendicularLengthSquared = (leftPerpendicularVector.x * leftPerpendicularVector.x) + (leftPerpendicularVector.y * leftPerpendicularVector.y);
    if (perpendicularLengthSquared < 1e-12) {
      return fallbackToReferencePoints(center, start, end, referencePoints);
    }

    const dotProduct = (midpointToCenter.x * leftPerpendicularVector.x) + (midpointToCenter.y * leftPerpendicularVector.y);

    if (Math.abs(dotProduct) > 1e-6) {
      return dotProduct > 0 ? 'CW' : 'CCW';
    }
