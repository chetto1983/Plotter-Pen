...
| `/api/cam/profile` | POST | Profile cut of the closed contours as PLC commands, through a piece of the given thickness on the bed or to a depth below its top; `closeGap` (0 to 1 mm) closes the gaps up to that width (same response as `/api/plc/extract`, plus `warnings` for contours the tool cannot reach and for open contours, and `open` with where each open contour runs and the closing gap that closes it; a drawing with only open contours is a 422 carrying `open`) |
...
| `/api/cam/operation` | GET/POST | The operation the PLC output panel showed before the job (pen, profile or drill, the piece, the parameters and the tools); the first step of an empty job is made from it |
